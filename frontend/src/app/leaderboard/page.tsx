export default function LeaderboardPage() {
  const podium = [
    { rank: 2, name: 'Luna Park', points: 3870, stage: 'Silver', color: 'from-slate-300 to-gray-400', emoji: '🥈' },
    { rank: 1, name: 'Arjun Shah', points: 4120, stage: 'Gold', color: 'from-amber-300 to-yellow-400', emoji: '🥇' },
    { rank: 3, name: 'Maya Chen', points: 3590, stage: 'Bronze', color: 'from-orange-300 to-amber-400', emoji: '🥉' },
  ]

  const rows = [
    { rank: 4, name: 'Leo Martin', points: 3310, stage: 'Bronze', progress: 78 },
    { rank: 5, name: 'Zara Khan', points: 3205, stage: 'Bronze', progress: 64 },
    { rank: 6, name: 'Noah Lee', points: 3090, stage: 'Rookie', progress: 52 },
    { rank: 7, name: 'Ava Patel', points: 2975, stage: 'Rookie', progress: 48 },
    { rank: 8, name: 'Ivy Gomez', points: 2860, stage: 'Rookie', progress: 42 },
  ]
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-10">
      <div className="container mx-auto px-4">
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-poppins font-bold text-gray-900">Leaderboard Stages</h1>
          <p className="text-gray-600 mt-2 font-inter">Climb from Rookie → Bronze → Silver → Gold and beyond!</p>
        </div>
            ...
            <div key={p.rank} className={`relative rounded-2xl shadow-lg overflow-hidden bg-gradient-to-br ${p.color} ${p.rank === 1 ? 'md:order-2' : p.rank === 2 ? 'md:order-1' : 'md:order-3'}`}>
              <div className="p-6 bg-white/70 backdrop-blur">
                ...
                <div className="mt-4">
                  ...
                  <div className="text-3xl font-extrabold text-gray-900">{p.points.toLocaleString()}</div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="divide-y">
            {rows.map((u) => (
              <div key={u.rank} className="px-6 py-4 grid grid-cols-12 items-center hover:bg-gray-50">
                <div className="col-span-2 font-semibold text-gray-800">#{u.rank}</div>
                <div className="col-span-4 flex items-center gap-3">
                  <span className="text-xl">{u.rank % 2 === 0 ? '🛡️' : '⚔️'}</span>
                  <div>
                    <div className="text-gray-900 font-semibold">{u.name}</div>
                    <div className="text-xs text-gray-500">Top {Math.max(1, 100 - u.rank)}%</div>
                  </div>
                </div>
                <div className="col-span-2">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${stageColor(u.stage)}`}>{u.stage}</span>
                </div>
                <div className="col-span-2 font-semibold text-gray-900">{u.points.toLocaleString()}</div>
                <div className="col-span-2">
